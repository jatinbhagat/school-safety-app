...
export async function getTenantReportingConfig(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const configResult = await pool.query(
      'SELECT config, updated_at FROM tenant_reporting_config WHERE tenant_id = $1',
      [finalTenantId]
    );
    ...
    console.error('Error fetching tenant reporting config:', error);
    res.status(500).json({
...
export async function updateTenantReportingConfig(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // TODO: Add auth check - verify user is tenant admin
    ...
    // TODO: Invalidate cache here if using Redis/in-memory cache
    ...
    console.error('Error updating tenant reporting config:', error);
    res.status(500).json({
      error: 'Internal server error',
